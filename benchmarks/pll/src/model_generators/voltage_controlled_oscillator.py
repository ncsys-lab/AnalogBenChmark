...
class VCO_Generator:
    ...
    def generate_block(input_voltage = 3.3, rel_prec = 0.01, timestep = 1e-2):
        ...
        gain = vco.decl_param("gain",Constant(0.5))
        ...
        x = vco.decl_var("x", kind=blocklib.VarKind.StateVar, type=RealType(lower=-0.1,upper=3.3,prec=rel_prec))
        ...
        dvdt = vco.decl_var("dvdt", kind=blocklib.VarKind.Transient,  \
                type=intervallib.real_type_from_expr(vco, (w*w)*(-Constant(-1.6) - x), rel_prec=rel_prec))
        dxdt = vco.decl_var("dxdt", kind=blocklib.VarKind.Transient, \
                type=intervallib.real_type_from_expr(vco, v, rel_prec=rel_prec))

        expr = VarAssign(dvdt, (w*w)*(- x - Constant(-1.6) ))
...
if __name__ == "__main__":
    ...
    rtl_block = rtllib.RTLBlock(int_block,{'x':0, 'v':0}, name_override = 'voltage_controlled_oscillator')
